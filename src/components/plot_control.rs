...
pub fn plotly_time_signal(props: &ControlProps) -> Html {
    ...
    let (control_variable, process_variable) = if open_loop {
        let y_no_feedback = 0.0; // open loop update
        // controller output u: aka control variable
        let u_open_loop: Array1<f64> = setpoint.iter().map(|sp| pid.update(y_no_feedback, *sp).unwrap_or(0.0)).collect();
        ...
        for i in 0..(dim-1) {
            u[i+1] = pid.update(setpoint[i], y[i]).unwrap_or(0.0);
            y[i+1] = element.transfer_td(u[i+1]);
        }
        ( u, y)
    };
                        ...
                        if let Some(signal) = props.signals.get(index) {
                            if props.elements.is_empty() {
                                ...
                                if let Some(element) = props.elements.get(index) {
                                    html! {
                                        <Item>
                                            <PlotControl range={props.range.clone()} signal={signal.clone()} element={element.clone()} />
                                        </Item>
                                    }
                                } else {
                                    html! { <Item>{"No elements available"}</Item> }
